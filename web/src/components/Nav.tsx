...
export function Nav() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-border bg-cream/85 px-6 py-3 backdrop-blur">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center border border-border-strong text-muted-foreground transition hover:bg-muted hover:text-ink cursor-pointer lg:hidden"
            title="Menu"
          >
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="sticky top-[57px] z-30 border-b border-border bg-cream/95 backdrop-blur lg:hidden">
          <nav className="grid grid-cols-2 gap-px bg-border p-px">
            {TABS.map((t) => {
              const active = t.to === "/" ? pathname === "/" : pathname.startsWith(t.to);
              return (
                <Link
                  ...
                  to={t.to}
                  className={cn(
                    "flex items-center bg-cream px-4 py-3 text-xs uppercase tracking-[0.15em] transition",
